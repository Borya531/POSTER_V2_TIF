...
class pyramid_trans_expr2(nn.Module):
    def __init__(self, img_size=224, num_classes=7, window_size=[28,14,7], num_heads=[2, 4, 8], dims=[64, 128, 256], embed_dim=768):
        ...
        import os
        _cwd = os.getcwd()
        os.chdir('/kaggle/working/Infant-Facial-Landmark-Detection-and-Tracking')  # 切换到 HRNet 代码所在目录，确保正确加载配置和权重
        ...
        os.chdir(_cwd)  # 立刻恢复，不影响 POSTER++ 的其他路径

        self.face_landback = HRNetLandmarkBackbone(
            hrnet_config    = hrnet_cfg,
            pretrained_path = '/kaggle/working/Infant-Facial-Landmark-Detection-and-Tracking/infanface_pretrained/hrnet-r90jt.pth',
        freeze_hrnet    = True,
        )
